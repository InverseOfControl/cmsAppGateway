...
public class AutoCancelDispatcher  extends  BaseController{

	protected static Log logger = LogFactory.getLog(AutoCancelDispatcher.class);
	@Autowired
	...
	public void execute() {
		logger.info("--------处于APP，未提交到录入修改队列的借款定时调用引擎动作：取消定时任务开始执行----------");
		...
		try {
			...
			if(count == 0){
				logger.info("成功：没有处于APP，未提交到录入修改队列的借款，日终跑批调用引擎动作：取消的单子");
				return;
			}
			//进行分页处理
...
//								app-SQXX	App待提交列表
					...
					if (EnumConstants.RES_CODE_SUCCESS.equals(resRule.getRepCode())) {
						...
						for (Map<String, Object> ruleMap : ruleMapList) {
							...
							if (APPEnumConstants.JOB_QX.equals(ruleMap.get("action"))) {
								// 如果请求成功并且响应信息为标识则更新app主表申请件为黄色
								Vo_700005 vo_700005 = new Vo_700005();
								vo_700005.setAppNo(ruleMap.get("loanNo").toString());
								vo_700005.setRefuseReason(ruleMap.get("actionCode").toString());
			...
			isException = true;
			logger.info("--------处于APP，未提交到录入修改队列的借款定时调用引擎动作：取消定时任务执行异常保存到Job日志中----------");
		} finally {
			logger.info("--------处于APP，未提交到录入修改队列的借款定时调用引擎动作：取消定时任务执行记录保存到Job日志中----------");
			logger.info("--------isException----------" + isException);
			logger.info("--------处于APP，未提交到录入修改队列的借款定时调用引擎动作：取消定时任务执行总条数----------" + count);
			logger.info("--------处于APP，未提交到录入修改队列的借款定时调用引擎动作：取消定时任务执行保存成功条数----------" + addCount);
		}
	}
