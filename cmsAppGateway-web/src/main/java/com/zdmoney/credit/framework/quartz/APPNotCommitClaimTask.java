 ...
 * @author 李璇
 * @date 2017年6月19日 下午1:59:24
 */
public class APPNotCommitClaimTask {
	protected static Log logger = LogFactory.getLog(APPNotCommitClaimTask.class);
	@Autowired
	...
	public void execute() {
		logger.info("--------APP环节发生客服认领的申请且未提交至录入修改队列的申请，日终跑批推送益百利做拒绝处理定时任务开始执行----------");
		...
		try {
			...
			map.put("isMoved", "Y");//客服认领标志
			//新展业标识，借款编号为14位
			map.put("appNoLength",14);
			
			count = iAPPNotCommitApplyService.queryNotCommitApplyAppNoCount(map);
			if(count == 0){
				logger.info("成功：没有APP环节发生客服认领的申请且未提交至录入修改队列的申请，日终跑批推送益百利做拒绝处理的单子");
				return;
			}
			//进行分页处理
...
//								app-SQXX	App待提交列表
			...
			isException = true;
			logger.info("--------APP环节发生客服认领的申请且未提交至录入修改队列的申请，日终跑批推送益百利做拒绝处理定时任务执行异常保存到Job日志中----------");
		} finally {
			logger.info("--------APP环节发生客服认领的申请且未提交至录入修改队列的申请，日终跑批推送益百利做拒绝处理定时任务执行记录保存到Job日志中----------");
			logger.info("--------isException----------" + isException);
			logger.info("--------APP环节发生客服认领的申请且未提交至录入修改队列的申请，日终跑批推送益百利做拒绝处理定时任务执行总条数----------" + count);
			logger.info("--------APP环节发生客服认领的申请且未提交至录入修改队列的申请，日终跑批推送益百利做拒绝处理定时任务执行保存成功条数----------" + addCount);
		}
	}
