...
<link href="<%=ctx%>/resources/static/css/common.css?<%=debug%>"
	rel="stylesheet" type="text/css" />
<link href="<%=ctx%>/resources/static/css/index.css?<%=debug%>"
	rel="stylesheet" type="text/css" />
...
<body>
	...
	<div class='imgtest' style='display: none'>
		...
		<span class='tishi'>提示：如果没有获取到验证码，请再次获取</span>
		<br />
	</div>
...
<script type="text/javascript"
	src="<%=ctx%>/resources/static/js/zepto.min.js?<%=debug%>"></script>
<script type="text/javascript"
	src="<%=ctx%>/resources/static/js/chinaMobile.js?<%=debug%>"></script>
<script type="text/javascript"
	src="<%=ctx%>/resources/static/js/jquery-1.11.3.min.js"></script>
	<script type="text/javascript"
	src="<%=ctx%>/resources/static/js/phone.js"></script>
<!-- <script
		 ...
		 $.ajax({
			 ...
			 success:function(data){ 
				 ...
				 if(data=='error'){
				    ...
				    var json=data;
				    var contextUrl="<%=ctx%>";
					$("#authCode").attr("src",contextUrl+"/"+json.respDesc);
				 }
			 }
		 });
	}
